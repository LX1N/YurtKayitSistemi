...
namespace YurtKayitSistemi
{
    public partial class FrmBolumler : Form
    {
        ...
        private void imgAdd_Click(object sender, EventArgs e)
        {
            ...
            //SELECT VE INSERT SORUSU !!!

            //SqlCommand cmd = new SqlCommand("select BolumAd from Bolumler where BolumAd=@k1",bgl.baglanti());
            //cmd.Parameters.AddWithValue("@k1", txtBolumAd.Text);
            //SqlDataReader dr = cmd.ExecuteReader();
            //while (dr.Read())
            //{
            //    if (dr[0].ToString() == txtBolumAd.Text)
            //    {
            //        MessageBox.Show("Test");
            //    }
            //    else
            //    {
            //        SqlCommand komut1 = new SqlCommand("insert into Bolumler (BolumAd) values (@k1)", bgl.baglanti());
            //        komut1.Parameters.AddWithValue("@k1", txtBolumAd.Text);
        ...
        private void imgDelete_Click(object sender, EventArgs e)
        {
            ...
            else if (txtBolumId.Text == "")
            {
                MessageBox.Show("Bölüm ID boş geçilemez.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                try
